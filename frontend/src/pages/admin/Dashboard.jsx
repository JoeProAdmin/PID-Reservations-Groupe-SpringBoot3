        ...
        { name: 'Prix', selector: row => `${row.price} €`, sortable: true },
        {
            ...
            cell: row => (
                <div className="d-flex gap-2">
                    <Link to={`/spectacles/${row.id}`}
                        className="btn btn-sm btn-dark text-warning"
                        style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 700, fontSize: '0.7rem' }}>
                        <i className="fas fa-eye"></i>
                    </Link>
                    <Link to={`/spectacles/${row.id}/edit`}
                        className="btn btn-sm btn-warning"
                        style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 700, fontSize: '0.7rem' }}>
                        <i className="fas fa-pen"></i>
                    </Link>
                    <button onClick={() => deleteSpectacle(row.id, row.title)}
        ...
        { name: 'Rôle', selector: row => row.role, sortable: true },
        {
            ...
            cell: row => (
                <div className="d-flex gap-2">
                    ...
                    <button onClick={() => deleteUser(row.id, row.email)}
                        className="btn btn-sm btn-outline-danger"
                        style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 700, fontSize: '0.7rem' }}>
                        <i className="fas fa-trash"></i>
                    </button>
                    ...
                    <div className="row g-4 mb-4">
                        <div className="col-md-3">
                            <div className="agency-card">
                                ...
                                <div className="card-content text-center">
                                    <i className="fas fa-theater-masks fa-2x mb-2" style={{ color: '#fec810' }}></i>
                                    <h3 style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 700 }}>{spectacles.length}</h3>
                        ...
                        <div className="col-md-3">
                            <div className="agency-card">
                                <div className="card-accent"></div>
                                <div className="card-content text-center">
                                    <i className="fas fa-users fa-2x mb-2" style={{ color: '#fec810' }}></i>
                                    <h3 style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 700 }}>{users.length}</h3>
